...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> initCastle() async {
    ...
    try {
      await Castle.instance.configure(
      ...
      token = await Castle.instance.createRequestToken;
      queueSize = await Castle.instance.queueSize;
      userAgent = await Castle.instance.userAgent;
      requestTokenHeaderName = await Castle.instance.requestTokenHeaderName;
      await Castle.instance.userJwt(
  ...
  void _userJwt() {
    Castle.instance.userJwt(
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImVjMjQ0ZjMwLTM0MzItNGJiYy04OGYxLTFlM2ZjMDFiYzFmZSIsImVtYWlsIjoidGVzdEBleGFtcGxlLmNvbSIsInJlZ2lzdGVyZWRfYXQiOiIyMDIyLTAxLTAxVDA5OjA2OjE0LjgwM1oifQ.eAwehcXZDBBrJClaE0bkO9XAr4U3vqKUpyZ-d3SxnH0"
    );
  }

  void _trackScreen() {
    Castle.instance.screen("Example screen");
  }

  void _trackCustom() {
    Castle.instance.custom("Custom event", null);
  }

  void _trackCustomProperties() {
    Castle.instance.custom("Custom event with properties", {
  ...
  void _reset() {
    Castle.instance.reset();
  }

  void _flush() {
    Castle.instance.flush();
  }
}
