...
void main() {
  ...
  test('createRequestToken', () async {
    expect(await Castle.instance.createRequestToken, 'abc123');
  });

  test('requestTokenHeaderName', () async {
    expect(await Castle.instance.requestTokenHeaderName, 'X-Castle-Request-Token');
  });

  test('userAgent', () async {
    expect(await Castle.instance.userAgent, 'castle_flutter_example/1.0 (1)(Castle 1.2.2; Android 11; Google sdk_gphone_x86_arm)');
  });

  test('queueSize', () async {
    expect(await Castle.instance.queueSize, 100);
  });
}
